...
def parseOpts(overrideArguments=None):
    def _readOptions(filename_bytes, default=[]):
        ...
        try:
            # FIXME: https://github.com/nextdl/nextdl/commit/dfe5fa49aed02cf36ba9f743b11b0903554b5e56
    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            ...
            userConfFile = os.path.join(
                compat_expanduser("~"), ".config", "nextdl", "config"
            )
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(
                    compat_expanduser("~"), ".config", "nextdl.conf"
                )
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, "nextdl", "config"), default=None
                )
                if userConf is None:
                    userConf = _readOptions(
                        os.path.join(appdata_dir, "nextdl", "config.txt"), default=None
                    )

        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "nextdl.conf"), default=None
            )
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "nextdl.conf.txt"), default=None
            )
