...
from ..utils import (KNOWN_EXTENSIONS, ExtractorError, HEADRequest,
                     UnsupportedError, determine_ext, float_or_none,
                     int_or_none, is_html, js_to_json, merge_dicts,
                     mimetype2ext, orderedSet, parse_duration,
                     sanitized_Request, smuggle_url, unescapeHTML,
                     unified_timestamp, unsmuggle_url, url_or_none, xpath_attr,
                    ...
                    if default_search == "auto_warning":
                        if re.match(r"^(?:url|URL)$", url):
                            raise ExtractorError(
                                'Invalid URL:  %r . Call nextdl like this:  nextdl -v "https://www.youtube.com/watch?v=BaW_jenozKc"  '
                                % url,
                                expected=True,
                            )
                        else:
